...
export default function DetailKosPriaModern({ onNavigateBack }) {
  ...
  // DATA DINAMIS
  const data = {
    name: "Kos Pria Modern Living",
    status: "2 Kamar Tersedia",
    statusColor: "bg-green-100 text-green-700",
    price: "980.000",
    address: "Jl. Keputih, Sukolilo, Surabaya Timur",
    facilities: [
      { label: "WiFi", icon: IconWifi },
      { label: "Parkir Motor", icon: IconParkir },
      { label: "Laundry", icon: IconLaundry }
    ],
    rules: [
      { label: "Dilarang Merokok", icon: IconNoSmoking },
      { label: "Tamu Max Jam 21.00", icon: IconJamMalam }
    ]
  };
    ...
    <div className="min-h-screen bg-[#F5F5F5] pb-24 text-gray-800">
      ...
      <div className="w-full max-w-3xl mx-auto bg-white mt-1">
        <div className="w-full h-56 md:h-80 overflow-hidden relative">
          <img src="https://images.unsplash.com/photo-1505693314120-0d443867891c?w=1000&q=80" alt="Kamar Utama" className="w-full h-full object-cover" />
        </div>
        <div className="flex w-full h-16 md:h-24">
          <img src="https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=300&q=80" alt="Thumb 1" className="w-1/4 h-full object-cover border-r border-white brightness-75 hover:brightness-100 transition-all cursor-pointer" />
          <img src="https://images.unsplash.com/photo-1598928506311-c55ded91a20c?w=300&q=80" alt="Thumb 2" className="w-1/4 h-full object-cover border-r border-white brightness-75 hover:brightness-100 transition-all cursor-pointer" />
          <img src="https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=300&q=80" alt="Thumb 3" className="w-1/4 h-full object-cover border-r border-white brightness-75 hover:brightness-100 transition-all cursor-pointer" />
          <div className="w-1/4 h-full relative cursor-pointer group bg-gray-900">
            <img src="https://images.unsplash.com/photo-1513694203232-719a280e022f?w=300&q=80" alt="Thumb 4" className="w-full h-full object-cover opacity-50" />
      ...
      <main className="max-w-3xl mx-auto p-4 md:p-6">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-3 mb-2">
            ...
            <span className={`${data.statusColor} px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide`}>{data.status}</span>
          </div>
        ...
        <div className="mb-8 bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="text-sm md:text-base text-gray-600 space-y-3 leading-relaxed">
            <p>Kos Pria Modern dengan desain minimalis, lingkungan asri, dan bersih. Pilihan tepat untuk mahasiswa maupun pekerja yang mencari kenyamanan dengan harga terjangkau.</p>
            <p><strong>Detail Lokasi:</strong> {data.address} (Masuk gang depan Indomaret Keputih, pagar hitam).</p>
            <p><strong>Fasilitas Kamar:</strong> Bangunan baru. Setiap Kamar berisi Kasur, Lemari 2 Pintu, Meja Belajar, Kursi, dan Jendela dengan ventilasi sangat baik.</p>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-3">
            {data.facilities.map((fac, idx) => (
              <div key={idx} className="bg-white border border-gray-200 px-4 py-2.5 rounded-xl flex items-center gap-2.5 shadow-sm">
                <img src={fac.icon} className="w-5 h-5 object-contain" alt={fac.label} />
          ...
          <div className="flex flex-wrap gap-3">
            {data.rules.map((rule, idx) => (
              <div key={idx} className="bg-white border border-amber-200 px-4 py-2.5 rounded-xl flex items-center gap-2.5 shadow-sm">
                <img src={rule.icon} className="w-5 h-5 object-contain" alt={rule.label} />
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm mb-10">
          ...
          <p className="text-sm text-gray-600 mb-5 leading-relaxed">
            Kosnya tenang banget, parkir motor luas, dan sangat aman. Mantap buat nugas sampai pagi tanpa gangguan.
          </p>
